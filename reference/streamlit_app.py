...
pipeline_path = Path(__file__).parent / 'models'
...
try: 
    from deep_forecast import DeepForecaster, train_test_split_ts
...
def run_forecast(
    ...
    try:
        ...
        elif module_type == "DeepForecast":
            if not DEEPFORECAST_AVAILABLE:
...
def main():
    ...
    if st.session_state.data_loaded and st.session_state.current_data is not None:
        ...
        if module_type == "StatsForecast" and strategy == "Multi-output direct":
            st.sidebar.warning("Multi-output is NOT supported for StatsForecast models")
